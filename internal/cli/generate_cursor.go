...
func generateCursorAt(scope installScope, root string, artifacts packArtifacts, opts generatorOptions) (generatorSummary, error) {
	...
	if scope == scopeGlobal {
		hookCommand = "hooks/guardrails.sh"
	}
	...
	for _, source := range artifacts.skills {
		if err := generateCursorSkillRule(scope, root, source, opts, &summary); err != nil {
			return summary, err
		}
	}
...
func generateCursorSkillRule(scope installScope, root, source string, opts generatorOptions, summary *generatorSummary) error {
