---
description: "Run the full technology-neutral coding workflow with phase-local skills: refine, spec, TDD, verification, review, docs, and commit."
---
...
Use `refine` only while material ambiguity remains, then use `spec` to produce a numbered folder under `specs/`
...
Use the `tdd` test-generation reference on the **Ready** spec. Choose unit or
...
Use `tdd` to implement one behavior at a time. Update `tasks.md` checkboxes.
...
After `verify` collects fresh evidence, load its independent review contract and
create a bounded read-only runtime subagent to verify alignment between spec,
tests, and code. Review inline only when the runtime cannot isolate a subagent.
...
Use the project-documentation mode of `docs` only if:
